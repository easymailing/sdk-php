...
final class BatchOperationsResource extends AbstractResource
{
    ...
    {
        $result = $this->client->request('GET', $this->resolvePath('/batch_operations', []), query: $query);
    ...
    {
        $result = $this->client->request('POST', $this->resolvePath('/batch_operations', []), body: is_array($body) ? $body : $body->toArray());
    ...
    {
        $result = $this->client->request('GET', $this->resolvePath('/batch_operations/{uuid}', ['uuid' => $uuid]));
        $data = is_array($result['data']) ? $result['data'] : [];
        return \Easymailing\Sdk\Generated\Dto\BatchOperation_batch_operation_read::fromArray($data);
    }
    ...
    {
        $this->client->request('DELETE', $this->resolvePath('/batch_operations/{uuid}', ['uuid' => $uuid]));
    }

    public function regenerateResponseBodyUrl(string $uuid): \Easymailing\Sdk\Generated\Dto\BatchOperationResource
    {
        $result = $this->client->request('PUT', $this->resolvePath('/batch_operations/{uuid}/actions/regenerate_response_body_url', ['uuid' => $uuid]));
        $data = is_array($result['data']) ? $result['data'] : [];
    ...
    {
        $result = $this->client->request('GET', $this->resolvePath('/batch_operations/{uuid}/errors', ['uuid' => $uuid]));
        $data = is_array($result['data']) ? $result['data'] : [];
